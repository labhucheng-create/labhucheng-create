...
<p align="center">
  我主要做底层驱动开发以及Ai infra，重点放在 CUDA Runtime、算子优化，以及擅长Linux驱动开发，主要针对于低高速总线以及VPU视频编解码驱动。目前主要在学习推理框架优化以及高性能算子开发。
</p>
...
Built a CUDA-like software stack around runtime interfaces, low-level software paths near the driver boundary, and operator-side performance work.

- Designed runtime-facing interfaces that expose low-level compute capability to upper-layer developers.
- Built low-level control and resource-management paths for device interaction, including event-style synchronization and library-side abstractions.
- Focused on operator optimization under a SIMT-style execution model, especially around parallel partitioning and compute/data-movement balance.

## Research

### AudioBias-Bench / Speech AI Research

Worked on speech-centric large-model bias evaluation through benchmark and dataset construction.

- Contributed to **AudioBias-Bench** and the **ECHO 22K** dataset for social-bias evaluation in speech-centric models.
- Designed tasks spanning speech recognition and generation, then analyzed model behavior across demographic dimensions.
